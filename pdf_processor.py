...
class FileProcessor:
                ...
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": "لطفاً سوالات و گزینه‌های موجود در این عکس را به صورت متن استخراج کن. فقط متن را برگردان."},
    ...
    def extract_text_from_pdf(file_path):
        ...
        # روش 2: برای فایل‌های اسکن‌شده (تمامی صفحات را به عکس تبدیل کن)
        if convert_from_path and GROQ_API_KEY:
            try:
                images = convert_from_path(file_path, dpi=200)
                full_text = ""
                for i, img in enumerate(images):
                    img_path = f"temp_ai_scan_{i}.jpg"
                    img.save(img_path)
                    page_text = FileProcessor.extract_text_from_image_ai(img_path)
                    os.remove(img_path)
                    if page_text:
                        full_text += page_text + "\n\n"
                return full_text.strip()
            except Exception as e:
                print(f"خطای هوش مصنوعی چندصفحه‌ای: {e}")
                pass
        return ""
