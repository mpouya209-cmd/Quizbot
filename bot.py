...
async def begin_private_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer() # پاسخ فوری به دکمه
    user_id = update.effective_user.id
...
# ========== شروع آزمون گروهی (اصلاح شده نهایی) ==========
async def begin_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer() # <--- مهم‌ترین خط برای اینکه دکمه بمیرد و کار نکند!
    
    # بررسی وجود پیام برای جلوگیری از کرش
    if not query.message:
        await query.edit_message_text("❌ خطا: پیام دکمه یافت نشد. لطفاً مجدداً تلاش کنید.")
        return
    ...
    if session: 
        return await query.edit_message_text("⏳ یک کوئیز در این گروه در حال اجراست!")
...
# ========== هندلر پاسخ‌ها (اصلاح شده نهایی) ==========
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer() # <--- پاسخ فوری برای جلوگیری از مردن دکمه
...
def main():
    ...
    # ===== متن (فقط پیوی) - تفکیک شده برای جلوگیری از تداخل =====
    ...
    if not RAILWAY_DOMAIN:
        # اگر محیط لوکال است، این را به آدرس ربات خود تغییر دهید
