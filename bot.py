...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("↗️ اشتراک آزمون", callback_data=f"share_inline_{quiz_id}")]
    ]
    await update.message.reply_text(
...
# ========== اشتراک‌گذاری با اینلاین (مثل عکس) ==========
async def share_inline_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    data = query.data.split('_')
    quiz_id = data[2]
    ...
    await query.edit_message_text(
        "📤 **ارسال آزمون به گروه/چت**\n\n"
        "لطفاً در کادر پایین، نام گروه یا مخاطب مورد نظر را تایپ کنید.\n"
        "سپس از لیست باز شده، گزینه مورد نظر را انتخاب کنید."
    )
    
    # ذخیره کوئیز در context.bot_data برای استفاده در InlineQuery
    context.bot_data['current_quiz_id'] = quiz_id

async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """وقتی کاربر در اینلاین تایپ می‌کند، این تابع لیست چت‌ها را نشان می‌دهد"""
    query = update.inline_query
    results = []
    
    # دریافت کوئیز آیدی از حافظه ربات
    quiz_id = context.bot_data.get('current_quiz_id', '')
    if not quiz_id:
        return
    ...
    keyboard = [
        [InlineKeyboardButton("این آزمون را شروع کنید", callback_data=f"begin_group_{quiz_id}")],
        [InlineKeyboardButton("آغاز آزمون در گروه", callback_data=f"group_start_{quiz_id}")],
    ...
    results = [
        InlineQueryResultArticle(
            id="share_quiz",
            title="📝 ارسال آزمون به این چت",
            description="این آزمون را به این گروه/چت ارسال کنید",
...
# ========== شروع آزمون گروهی از طریق لینک ==========
async def start_group_share(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """وقتی کاربر لینک اشتراک را در گروه می‌فرستد، این تابع اجرا می‌شود"""
    if update.effective_chat.type not in ['group', 'supergroup']:
        return
    text = update.message.text
    match = re.search(r'start=group_([a-f0-9]+)', text)
    if not match:
        return
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(share_inline_callback, pattern="^share_inline_"))
    ...
    # ===== هندلر لینک گروه =====
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, start_group_share))

    logger.info("🤖 ربات با اشتراک‌گذاری اینلاین روشن شد!")
