...
# ========== شروع آزمون گروهی (نسخه تضمینی با هندل کردن خطا) ==========
async def begin_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    
    try:
        # امن‌ترین روش برای گرفتن آیدی گروه در تلگرام
        if query.message:
        ...
        if not chat_id:
            await query.edit_message_text("❌ خطا: آیدی گروه پیدا نشد.")
            return
        ...
        data = query.data.split('_')
        if len(data) < 3:
            await query.edit_message_text("❌ خطا در شناسایی آزمون.")
            return
            
        quiz_id = data[2]
        await db.save_group_session(chat_id, quiz_id, 0)
        
        # تغییر پیام به جای ادیت کردن آن (برای جلوگیری از خطای دکمه)
        await query.edit_message_text("✅ لینک شروع آزمون دریافت شد! در حال شروع...", reply_markup=None)
        
        # فراخوانی نمایش سوال اول
        await show_question_group(update, context, chat_id, quiz_id, 0)
        
    except Exception as e:
        logger.error(f"خطا در شروع آزمون گروهی: {e}")
        try:
            await query.edit_message_text(f"❌ خطای سیستمی در شروع آزمون رخ داد. لطفاً با پشتیبان تماس بگیرید.")
        except:
            pass
...
async def show_question_group(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id, quiz_id, index):
    try:
        ...
        # ارسال نظرسنجی با هندل کردن خطا
        ...
        logger.error(f"خطا در نمایش سوال گروهی: {e}")
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.error(f"خطا در هندل کردن پاسخ نظرسنجی: {e}")
