...
# دیکشنری برای ذخیره لیست اعضای آماده
group_ready_users = {}
...
async def begin_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== پاک کردن جلسه قبلی اگر مانده باشد =====
...
async def group_ready_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data[0] == 'group_ready_user':
        ...
        # ===== اگر کاربر قبلاً ثبت‌نام کرده، حذفش کن (-1) =====
        else:
            group_ready_users[chat_id]['users'].remove(user_id)
            count = len(group_ready_users[chat_id]['users'])
            
            new_text = f"{query.message.text}\n\n❌ شما انصراف دادید! (آماده: {count} نفر)"
            await query.edit_message_text(new_text, reply_markup=query.message.reply_markup)
            
            # اگر تعداد کمتر از ۲ شد، دکمه شروع رو حذف کن (چون شرط دیگه برقرار نیست)
            # (فعلاً این قسمت رو ساده نگه می‌داریم تا منطق پیچیده نشه)
...
async def group_admin_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != creator_id:
        await query.edit_message_text("❌ فقط فرستنده سوالات می‌تواند آزمون را شروع کند!")
        return
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== چک کردن دکمه‌های گروهی (باید اول باشه) =====
...
def main():
    ...
    logger.info("🤖 ربات با دکمه +1 و -1 و نمایش تعداد روشن شد!")
