...
# ========== هندلر اینلاین (مهم‌ترین بخش) ==========
async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.inline_query
    query_text = query.query
    
    # بررسی اینکه آیا کاربر با quiz: شروع کرده
    if not query_text.startswith("quiz:"):
        return

    # استخراج آیدی کوئیز از متن
    quiz_id = query_text[5:].strip()
    questions = db.get_questions(quiz_id)
    if not questions:
        return

    # ساخت باکس اطلاعات (دقیقاً مثل عکس)
    text = (
        f"📚 **دروس قانون اساسی**\n\n"
        f"**برای آزمون آماده شوید!**\n\n"
        f"✍️ تعداد سوالات: **{len(questions)}** سوال\n"
    ...
    # وقتی کاربر روی این باکس کلیک کنه، مستقیم آزمون شروع میشه
    results = [
        InlineQueryResultArticle(
            id="start_quiz",
            title="شروع آزمون گروهی",
            description="روی این گزینه کلیک کنید تا آزمون در گروه شروع شود",
            input_message_content=InputTextMessageContent("🎯 لینک شروع آزمون دریافت شد. در حال راه‌اندازی..."),
...
def main():
    ...
    # ===== هندلر اینلاین (برای نمایش باکس اطلاعات) =====
    ...
    logger.info("🤖 ربات با باکس اینلاین (مثل عکس) روشن شد!")
