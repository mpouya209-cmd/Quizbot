...
async def begin_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query.message:
        await query.answer(
            "❌ این دکمه منقضی شده است. لطفاً مجدداً آزمون را از طریق لینک اشتراک شروع کنید.",
            show_alert=True
        )
        return
    ...
    try:
        ...
        # بررسی وجود سوالات در دیتابیس
        questions = db.get_questions(quiz_id)
        if not questions:
            await query.edit_message_text("❌ سوالات این آزمون یافت نشد!")
            return
        ...
        await query.edit_message_text(
            f"🎯 آزمون گروهی شروع شد!\n\n📝 تعداد سوالات: {len(questions)}",
            reply_markup=None
        )
        ...
        logger.error(f"Error in begin_group_callback: {e}")
        await query.edit_message_text("❌ خطا در شروع آزمون. لطفاً دوباره تلاش کنید.")
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اگر خطا مربوط به NoneType بود، به کاربر اطلاع بده
    if isinstance(context.error, AttributeError) and "'NoneType' object has no attribute" in str(context.error):
        try:
