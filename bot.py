...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type in ['group', 'supergroup']:
        await update.message.reply_text(
            "🤖 **ربات کوئیز در گروه**\n\n"
            "📤 برای ارسال فایل سوالات و ساخت آزمون، لطفاً به پیوی ربات مراجعه کنید.\n"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔗 اشتراک آزمون در گروه", callback_data=f"share_group_{quiz_id}")]
    ]
    await update.message.reply_text(
...
# ========== اشتراک آزمون در گروه ==========
async def share_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    data = query.data.split('_')
    quiz_id = data[2]
    ...
    await query.edit_message_text(
        f"🔗 **لینک اشتراک آزمون در گروه**\n\n"
        f"برای شروع آزمون در گروه، این لینک را در گروه بفرستید:\n"
        f"`{share_link}`"
    )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(share_group_callback, pattern="^share_group_"))
    ...
    logger.info("🤖 ربات دوحالته (پیوی کامل + گروه فقط لینک) روشن شد!")
