...
# ========== بخش جدید: دستور گروهی ==========
async def group_start_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not row:
        await update.message.reply_text("📤 ابتدا یک فایل سوالات را به گروه بفرستید یا از طریق /newquiz بسازید.")
        return

    quiz_id = row[0]
    questions = db.get_questions(quiz_id)
    if not questions:
        await update.message.reply_text("❌ سوالی در دیتابیس پیدا نشد!")
        return

    # ذخیره کوئیز برای گروه + آیدی فرستنده
    await db.save_group_session(chat_id, quiz_id, 0)
    # در یک دیکشنری جداگانه، آیدی شروع‌کننده رو ذخیره می‌کنیم
...
async def group_ready_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data[0] == 'group_ready_user':
        ...
        # اگر این گروه هنوز توی دیکشنری ثبت نشده، مقداردهی اولیه کن
        if chat_id not in group_ready_users:
        ...
        # اگر تعداد شرکت‌کنندگان >= ۲ بود، فقط برای فرستنده (creator) دکمه شروع را نشان بده
        if len(group_ready_users[chat_id]['users']) >= 2:
            # دکمه شروع فقط برای کسی که دستور groupstart رو زده
            ...
            # پیام رو فقط برای فرستنده می‌فرستیم (با استفاده از آیدی که ذخیره کردیم)
...
async def group_admin_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # پاک کردن لیست آمادگی بعد از شروع
...
async def show_question_group(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id, quiz_id, index):
    ...
    q = questions[index]
    options = q['options'][:4]  # فقط ۴ گزینه اول را نگهدار
...
def main():
    ...
    # ===== هندلرهای دستورات (باید اول باشند) =====
    ...
    app.add_handler(CommandHandler("begin", group_start_admin))

    # ===== هندلرهای فایل و عکس =====
    app.add_handler(MessageHandler(filters.Document.ALL, handle_document))
    app.add_handler(MessageHandler(filters.PHOTO, handle_photo))

    # ===== هندلرهای متن (باید بعد از دستورات باشند) =====
    ...
    logger.info("🤖 ربات با دستور /begin روشن شد!")
