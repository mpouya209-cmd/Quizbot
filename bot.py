...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(0, len(questions), 100):
        await db.save_questions(quiz_id, questions[i:i+100])
    keyboard = [
        [InlineKeyboardButton("🎯 شروع آزمون در پیوی", callback_data=f"begin_private_{quiz_id}")],
        [InlineKeyboardButton("↗️ اشتراک آزمون", switch_inline_query=f"quiz:{quiz_id}")]
    ]
    await update.message.reply_text(
...
async def handle_text_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ {len(questions)} سوال از متن استخراج شد!\n\n"
        f"حالا می‌توانید:",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
# ========== شروع آزمون گروهی با منطق پیوی و ظاهر Poll ==========
async def begin_group_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ذخیره جلسه با user_id (مثل پیوی)
    await db.save_session(user_id, quiz_id, 0, [])
    await query.edit_message_text("🎯 آزمون گروهی شروع شد! سوال اول را ببینید.")
    # نمایش سوال با Poll به جای دکمه
    await show_question_group(update, context, user_id, chat_id, quiz_id, 0)
...
async def inline_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"✍️ تعداد سوالات: **{len(questions)}**\n"
        f"⏱️ زمان: **{TIMER_SECONDS}** ثانیه به ازای سوال\n"
    ...
    results = [
        InlineQueryResultArticle(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🎯 شروع آزمون", callback_data=f"begin_group_{quiz_id}")]
            ]),
...
# ========== نمایش سوالات گروهی با Poll ==========
async def show_question_group(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id, chat_id, quiz_id, index):
    questions = db.get_questions(quiz_id)
    if index >= len(questions): 
        return await finish_group_quiz(update, context, user_id, chat_id, quiz_id)
    
    q = questions[index]
    options = q['options'][:4]
    
    # ارسال Poll به جای دکمه
    ...
    active_timers[chat_id] = asyncio.create_task(start_group_timer(update, context, user_id, chat_id, quiz_id, index))
...
async def start_group_timer(update, context, user_id, chat_id, quiz_id, index):
    try:
        ...
        if session and session['quiz_id'] == quiz_id:
            ...
            questions = db.get_questions(quiz_id)
            if next_index >= len(questions):
                await finish_group_quiz(update, context, user_id, chat_id, quiz_id)
            else:
                await db.save_session(user_id, quiz_id, next_index, [])
                await show_question_group(update, context, user_id, chat_id, quiz_id, next_index)
    except asyncio.CancelledError: pass
    finally:
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ذخیره پاسخ با user_id (مثل پیوی)
    session = db.get_session(user_id)
    if not session:
        await db.save_session(user_id, quiz_id, 0, [])
        session = db.get_session(user_id)
    ...
    answers = session['answers']
    while len(answers) <= current_index: answers.append(-1)
    answers[current_index] = selected
    await db.save_session(user_id, quiz_id, current_index + 1, answers)

# ========== هندلر پاسخ‌ها (اصلاح شده - خط 333 حذف شد) ==========
...
async def finish_group_quiz(update, context, user_id, chat_id, quiz_id):
    if chat_id in active_timers:
        active_timers[chat_id].cancel()
    ...
    # دریافت پاسخ‌های گروهی از دیتابیس
    db.cursor.execute("SELECT user_id, answers FROM group_answers WHERE chat_id=? AND quiz_id=?", (chat_id, quiz_id))
    ...
    for row in rows:
        ...
        for i, q in enumerate(questions):
            if i < len(answers) and answers[i] == q['correct_answer']:
                score += 1
        try:
            member = await context.bot.get_chat_member(chat_id, user_id_db)
            name = member.user.first_name
        except:
            name = f"User {user_id_db}"
        results.append({'user_id': user_id_db, 'name': name, 'score': score})
...
def main():
    ...
    # ===== متن (فقط پیوی) =====
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVATE, handle_text_quiz))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVATE, handle_quiz_builder))

    # ===== هندلر نظرسنجی (PollAnswer) =====
    app.add_handler(PollAnswerHandler(handle_poll_answer))

    # ===== اینلاین هندلر (باکس شیشه‌ای) =====
    ...
    logger.info("🤖 ربات با دکمه شروع گروهی فعال روشن شد!")
