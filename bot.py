...
# ========== شروع آزمون گروهی (فقط برای گروه) ==========
...
def main():
    ...
    # ===== ۱. دستورات عمومی (مشترک بین پیوی و گروه) =====
    app.add_handler(CommandHandler("start", start))

    # ===== ۲. هندلرهای اختصاصی پیوی (فقط برای چت‌های خصوصی) =====
    # این هندلرها فقط وقتی کاربر در پیوی است فعال می‌شوند
    app.add_handler(CommandHandler("newquiz", new_quiz, filters=filters.ChatType.PRIVATE))
    app.add_handler(CommandHandler("done", done_quiz, filters=filters.ChatType.PRIVATE))
    app.add_handler(CommandHandler("cancel", cancel_quiz, filters=filters.ChatType.PRIVATE))
    
    app.add_handler(MessageHandler(filters.Document.ALL & filters.ChatType.PRIVATE, handle_document))
    app.add_handler(MessageHandler(filters.PHOTO & filters.ChatType.PRIVATE, handle_photo))
    
    # این دو هندلر متن برای پیوی هستند و در گروه فعال نمی‌شوند
    ...
    # ===== ۳. هندلرهای اختصاصی گروه (فقط برای آزمون دادن) =====
    # در گروه، هر متن عادی که کاربر بفرستد، ربات هیچ واکنشی نشان نمی‌دهد (ignored)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.GROUPS, lambda u, c: None))
    app.add_handler(MessageHandler(filters.PHOTO & filters.ChatType.GROUPS, lambda u, c: None))
    app.add_handler(MessageHandler(filters.Document.ALL & filters.ChatType.GROUPS, lambda u, c: None))
    
    # ===== ۴. هندلرهای مشترک برای هر دو بخش =====
    app.add_handler(PollAnswerHandler(handle_poll_answer))
    app.add_handler(InlineQueryHandler(inline_query_handler))

    # ===== ۵. دکمه‌ها (CallbackQuery) =====
