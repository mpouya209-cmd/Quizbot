...
async def group_ready_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data[0] == 'group_ready_user':
        ...
        # ===== وقتی تعداد به ۱ نفر رسید، دکمه شروع فقط برای فرستنده =====
        if len(group_ready_users[chat_id]['users']) >= 1:
            ...
            # فقط فرستنده می‌تواند این دکمه را ببیند (به شرط اینکه ادمین باشد یا همان کاربر)
            await context.bot.send_message(
                chat_id,
                "🎯 حداقل ۱ نفر آماده هستند. فرستنده سوالات می‌تواند آزمون را شروع کند!",
