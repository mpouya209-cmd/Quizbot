...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        questions = FileProcessor.parse_questions_from_text(text)
        if not questions:
            await update.message.reply_text("❌ از متن استخراج شده، سوالی پیدا نشد!")
            return
...
async def quiz_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اگر در گروه باشد، حالت گروهی اجرا می‌شود
...
def main():
    ...
    app.add_handler(MessageHandler(filters.PHOTO, handle_photo)) # هندل کردن عکس‌ها
